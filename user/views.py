...
def login_view(request):
    ...
    if user:
        ...
        response.set_cookie("token", token.decode('utf-8'), httponly=True, samesite="strict", domain="localhost", max_age=3600)
        return response
    response = JsonResponse({"response": "failure", "error": "Bad email/password"})
